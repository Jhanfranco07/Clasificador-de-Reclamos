    ...
    <div className="container mx-auto max-w-4xl px-4 py-8">
      ...
      {items.length === 0 ? (
        ...
        <div className="space-y-4">
          {items.map((item) => <article key={item.id} className="flex gap-4 rounded-lg border border-border bg-card p-4 text-card-foreground shadow-sm"><ImageWithFallback src={item.image} alt={item.name} className="size-20 rounded-md object-cover" /><div className="flex-1"><div className="flex justify-between gap-3"><div><h2 className="font-semibold">{item.name}</h2><p className="text-sm text-muted-foreground">{item.restaurantName}</p></div><Button variant="ghost" size="icon" onClick={() => save(items.filter((current) => current.id !== item.id))}><Trash2 className="size-4" /></Button></div><div className="mt-3 flex items-center justify-between"><div className="flex items-center gap-2"><Button variant="outline" size="icon" onClick={() => change(item.id, -1)}><Minus className="size-4" /></Button><span>{item.quantity}</span><Button variant="outline" size="icon" onClick={() => change(item.id, 1)}><Plus className="size-4" /></Button></div><strong>{formatCurrency(item.price * item.quantity)}</strong></div></div></article>)}
